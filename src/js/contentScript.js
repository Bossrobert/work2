...
if (matchDomain("elmercurio.com")) {
	...
	const articleBodySubscribed = document.querySelector(
		".c-article-body--subscribed"
	);
	if (articleBodySubscribed) {
		articleBodySubscribed.removeAttribute("class");
	}
} else if (
	matchDomain(["examiner.com.au", "thecourier.com.au", "theadvocate.com.au"])
) {
	const subscribeTruncate = document.querySelector(".subscribe-truncate");
	if (subscribeTruncate) {
		subscribeTruncate.classList.remove("subscribe-truncate");
	}
	...
	const paywall = document.querySelector(
		".subscribe-article.news-article-body.article__body"
	);
	...
	document.onreadystatechange = function () {
		if (document.readyState === "complete") {
			document
				.querySelector(".css-mcm29f")
				.setAttribute("style", "position:relative");
		}
	};
} else if (matchDomain("technologyreview.com")) {
	window.setTimeout(function () {
		const bodyObscured = document.querySelector(
			'body[class*="body__obscureContent"]'
		);
		if (bodyObscured) {
			removeClassesByPrefix(bodyObscured, "body__obscureContent");
		}
		const overlay = document.querySelector(
			'div[class*="overlayFooter__wrapper"]'
		);
		if (overlay) {
			overlay.setAttribute("style", "display:none");
		}
		const contentBodyHidden = document.querySelector(
			'div[class*="contentBody__contentHidden"]'
		);
		if (contentBodyHidden) {
			removeClassesByPrefix(contentBodyHidden, "contentBody__contentHidden");
		}
		const contentBodyOverlay = document.querySelector(
			'div[class*="contentBody__overlay"]'
		);
		if (contentBodyOverlay) {
			contentBodyOverlay.removeAttribute("class");
		}
	}, 500);
} else if (matchDomain("leparisien.fr")) {
	window.removeEventListener("scroll", this.scrollListener);
	const paywall = document.querySelector(
		".relative.piano-paywall.below_nav.sticky"
	);
	removeDOMElement(paywall);
	setTimeout(function () {
	...
	const paywall = document.querySelector(
		".tls-subscriptions-banner__closed-skin"
	);
	removeDOMElement(paywall);
	...
	const container = document.querySelector(
		".HardPayWallContainer-module__overlay"
	);
	window.setTimeout(function () {
	...
	if (!url.includes("barrons.com/amp/")) {
		...
		const signinLinks = document.querySelectorAll(
			'a.primary-button--link[href*="target="]'
		);
		for (const signinLink of signinLinks) {
			signinLink.href = decodeURIComponent(
				signinLink.href.split("target=")[1]
			).split("?")[0];
		...
		const subscrSection = document.querySelector(
	...
	document.addEventListener("DOMContentLoaded", () => {
		...
		const url = window.location.href;
		if (url.includes("mode=sample") || stubArticleMsg) {
	...
	window.setTimeout(function () {
		...
		if (aboBanner) {
			...
			try {
				...
				const article =
					data.article.data.stripes[0].mainContent[0].data.description;
				...
				if (paywallNode) {
					...
					if (articlePar) {
						...
						const paywallLastChildNode = document.querySelector(
							".post-paywall  > :last-child"
						);
						if (paywallLastChildNode) {
							paywallLastChildNode.setAttribute(
								"style",
								"height: auto !important; overflow: hidden !important; max-height: none !important;"
							);
						}
					}
				}
	...
	window.setTimeout(function () {
		document
			.querySelectorAll("[data-fa-message-dismiss-counter]")
			.forEach(function (el) {
				removeDOMElement(el);
			});
		const body = document.querySelector("body");
		...
		document
			.querySelectorAll('[class^="paywall"],[class*="paywall"]')
			.forEach(function (el) {
				removeDOMElement(el);
			});
	...
	setTimeout(function () {
		...
		const banner = document.querySelector(
			"#themarker\\.com\\.billboard\\.desktop"
		);
		const newsBanner = document.querySelector(
			"#themarker\\.com\\.news\\.banner"
		);
		const midBanner = document.querySelector(
			"#themarker\\.com.headline\\.banner\\.desktop"
		);
		const financeBanner = document.querySelector(
			"#themarker\\.com\\.finance\\.banner"
		);
		const topStrip = document.querySelector('[data-test="topStrip"]');
		const otherBanners = Array.from(
			document.querySelectorAll("[data-audtarget]")
		);
		removeDOMElement(
			paywall,
			notifications,
	...
	setTimeout(function () {
		...
		const editorsBanner = document.querySelector(
			"#haaretz\\.co\\.il\\.editors\\.banner"
		);
		const headlinesBanner = document.querySelector(
	...
	document.addEventListener("DOMContentLoaded", () => {
		...
		if (
			url.startsWith("https://www.sueddeutsche.de") &&
			(url.includes("reduced=true") || offerPage)
		) {
			window.location.href = url.split("?")[0].replace("www.", "amphtml.");
		} else if (url.startsWith("https://sz-magazin.sueddeutsche.de")) {
			if (url.includes("reduced=true") || offerPage) {
	...
	window.setTimeout(function () {
		if (url.includes("!amp")) {
			...
			if (paragraphReduced) {
				paragraphReduced.classList.remove("paragraph--reduced");
			}
			const paragraphHidden = document.querySelectorAll(".paragraph--hidden");
			for (const parHidden of paragraphHidden) {
				parHidden.classList.remove("paragraph--hidden");
			}
			const paragraphDynamic = document.querySelector(".paragraph--dynamic");
			if (paragraphDynamic) {
				paragraphDynamic.classList.remove("paragraph--dynamic");
			}
	...
	if (!url.includes("/amp." + "elmundo.es" + "/")) {
		...
		const paywall = document.querySelector(
			'div[amp-access="authorized!=true"]'
		);
		if (paywall) {
			removeDOMElement(paywall);
			const divHidden = document.querySelector(
				'div[amp-access="authorized=true"]'
			);
			if (divHidden) {
...
} else if (matchDomain("marketchameleon.com")) {
	// Remove the premium popup blocker
	const premiumPopArray = document.querySelectorAll("div.dis-prem-pop");
	if (premiumPopArray) {
	...
	// Get a list of the SELECT being DISABLED
	document.querySelectorAll("select[disabled]").forEach(function (el) {
